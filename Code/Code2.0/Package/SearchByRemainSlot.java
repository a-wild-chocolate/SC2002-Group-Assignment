...
public class SearchByRemainSlot extends Searcher {
	...
	public ArrayList<Camp> searching(ArrayList<Camp> campList) {
		...
		{
			if(!slotList.contains(camp.getRemainSlot()))
			{
				slotList.add(camp.getRemainSlot());
			}
		}
		...
		System.out.println();
		System.out.println("Please enter the number of remain slot you want to filter: (-1 quit)");
		while(true){
			choice=sc.nextInt();
			//Quit
			...
			{
				System.out.println("Sorry, there is no camp with "+choice+" remain slot . Please enter again");
				continue;
			}
			//generate the list
			else
