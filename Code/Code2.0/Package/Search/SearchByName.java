...
public class SearchByName extends Searcher {
	...
	public ArrayList<Camp> searching(ArrayList<Camp> campList) {
		...
		{
			if(!slotList.contains(camp.getCampName()))
			{
				slotList.add(camp.getCampName());
			}
		}
		//print all possible choice
		System.out.println("The available name list shows below:");
		...
		while(true){
			System.out.println("Please enter the name number you want to filter: (-1 quit)");
			choice=sc.nextInt();
			//Quit
			...
			else
			{
				for (Camp camp:campList)
				{
					if(camp.getCampName().equals(slotList.get(choice-1)))
					{
						returnList.add(camp);
					}
				}
